...
public class main {
	...
	public static void main(String[] args) {
		...
		ui.Upd.addActionListener(new ActionListener() {
	...
	private static void setUpdate() {
		String Command = "UPDATE bookDB SET bookName = '" + ui.bookNameUpdArea.getText() + "' WHERE bookName = '"
				+ ui.bookIdUpdArea.getText() + "'";

		try {
			if (!ui.bookIdUpdArea.getText().equals("") && !ui.bookNameUpdArea.getText().equals("")) {
				stm.executeUpdate(Command);
				ui.showUpdResult.setText("Update successful");
			} else {
				ui.showUpdResult.setText("not null");

			}

		} catch (SQLException e) {
			ui.showUpdResult.setText("error");
			e.printStackTrace();
		}
